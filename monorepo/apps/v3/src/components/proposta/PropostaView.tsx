import {
  sectionPublicNavLabel,
  visibleSectionsForTemplate,
  type CmsTemplateId,
...
import { ClientsProofSection } from "./ClientsProofSection";
import { CaseSpotlightSection } from "@/components/meups/CaseSpotlightSection";
import { PropostaAmbientBackdrop } from "./PropostaAmbientBackdrop";
import { PropostaScripts } from "./PropostaScripts";
import { PropostaPrintPdfButton } from "./PropostaPrintPdfButton";
import { UmSonAoFazBand } from "./UmSonAoFazBand";
      ...
      <div className="global-reach__inner">
        ...
        <div className="global-reach__left">
          <h2 className="global-reach__heading">
            Nosso trabalho não tem fronteiras
          </h2>
...
const LOADER_CLIENT_LOGO_FALLBACK = "/03%20-ciee-logo.svg";
const LOADER_CP_LOGO_URL = "/logo-cp.svg";

function loaderClientLogoUrl(p: Proposta): string {
  const url = p.cliente.logoUrl?.trim();
  if (url) return url;
  if (p.slug === "cp") return LOADER_CP_LOGO_URL;
  return LOADER_CLIENT_LOGO_FALLBACK;
}
...
export function PropostaView({
  ...
  const orderedSections = visibleSectionsForTemplate(
    template,
    p.sectionOrder,
    p.sectionHidden,
  );
  ...
  return (
    <>
      {p.slug === "cp" ? (
        <span className="proposta-yellow-theme-marker" aria-hidden="true" hidden />
      ) : null}
      ...
      <div className="proposta-main" id="proposta-export-root">
      <nav>
        ...
        <div className="nav-shell">
          ...
          <PropostaPrintPdfButton
            suggestedPdfTitle={`Proposta · ${p.proposta.numero} · ${p.cliente.nome}`}
          />
        </div>
      </nav>
      ...
      <div id="proposta-cases-fold" className="proposta-cases-fold" aria-label="Cases de referência Matilha">
        <hr className="divider" />
        <CaseSpotlightSection mode={p.slug === "educacao" ? "education" : "default"} />
        <hr className="divider" />
      </div>

      <ClientsProofSection
        files={clientLogoFiles}
        variant={p.slug === "educacao" ? "education" : "default"}
      />

      <hr className="divider" />

      {orderedSections.map((key) => (
        <SectionBlock key={key} sectionKey={key} data={p} template={template} />
      ))}
      ...
      <footer>
        ...
        <p
          className="contact-row proposta-footer__phone-row"
          style={{ fontSize: "0.95rem", color: "var(--text-soft)" }}
        >
          <a
            href="https://wa.me/5541997376060"
            target="_blank"
            rel="noopener noreferrer"
            className="proposta-footer__wa"
            aria-label="WhatsApp Matilha"
          >
            <span className="proposta-footer__wa-icon" aria-hidden="true">
          ...
          <span className="proposta-footer__phone-sep" aria-hidden="true">
            ·
          </span>
          <span>+55 41 99737-6060</span>
        </p>
...
function SectionBlock({
  ...
  return (
    <div className="section-group" data-section={sectionKey}>
      ...
      {renderSection(sectionKey, p, template)}
    </div>
  );
}
  ...
  switch (sectionKey) {
    ...
    case "equipe":
      ...
      <section id="equipe" className="fade-in">
        ...
        <div
          className={`cards-3${
            p.equipe.membros.length >= 6 ? " cards-3--six" : ""
          }`}
        >
          {p.equipe.membros.map((m, i) => (
            <div className="card equipe-servico-card" key={i}>
              {m.icone ? (
                <div className="equipe-servico-card__icone" aria-hidden="true">
                  {m.icone}
                </div>
              ) : null}
